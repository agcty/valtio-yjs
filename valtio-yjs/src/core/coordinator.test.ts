...
describe('ValtioYjsCoordinator', () => {
  ...
  describe('Arrays with Delta During Sync', () => {
    it('initially no arrays are marked with delta', () => {
      ...
      expect(coordinator.shouldSkipArrayStructuralReconcile(yArray)).toBe(false);
    });
    ...
    it('setArraysWithDeltaDuringSync marks arrays', () => {
      ...
      expect(coordinator.shouldSkipArrayStructuralReconcile(yArray1)).toBe(true);
      expect(coordinator.shouldSkipArrayStructuralReconcile(yArray2)).toBe(true);
    });
    ...
    it('only marked arrays return true for shouldSkip', () => {
      ...
      expect(coordinator.shouldSkipArrayStructuralReconcile(yArray1)).toBe(true);
      expect(coordinator.shouldSkipArrayStructuralReconcile(yArray2)).toBe(false);
    });
