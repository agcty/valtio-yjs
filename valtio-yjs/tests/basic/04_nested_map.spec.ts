...
describe('nested map operations', () => {
  it('nested map direct property set', async () => {
    ...
    expect(asYMap(asYMap(m.get('items')).get('item1')).get('color')).toStrictEqual('red');
  });
  ...
  it('nested map array property replace', async () => {
    ...
    expect((asYMap(asYMap(m.get('items')).get('item1')).get('point') as Y.Array<unknown>).toJSON()).toStrictEqual([100, 100]);
  });
  ...
  it('nested map uses yjs value on initialization', async () => {
    ...
    const { proxy: proxy1, bootstrap: bootstrap1 } = createYjsProxy<{ items: { item1: { color: string } } }>(doc1, {
      getRoot: (d) => d.getMap('root'),
    });
    ...
    await waitMicrotask();
    expect(asYMap(asYMap(map1.get('items')).get('item1')).get('color')).toStrictEqual('red');

    const { proxy: proxy2 } = createYjsProxy<{ items: { item1: { color: string } } }>(doc2, {
      getRoot: (d) => d.getMap('root'),
    });
    await waitMicrotask();

    expect(asYMap(asYMap(map2.get('items')).get('item1')).get('color')).toStrictEqual('red');
  ...
  it('creates a reasonable number of updates on bootstrap', async () => {
    ...
    const { proxy: proxy1, bootstrap: bootstrap1 } = createYjsProxy<{ items: Record<string, { color: string }> }>(doc1, {
      getRoot: (d) => d.getMap('root'),
    });
    ...
    const { proxy: proxy2 } = createYjsProxy<{ items: Record<string, { color: string }> }>(doc2, {
      getRoot: (d) => d.getMap('root'),
    });
    await waitMicrotask();
  ...
  it('nested map rejects undefined values', async () => {
    const doc = new Y.Doc();
    const { proxy: p } = createYjsProxy<{ a?: { b: number; c: string | null } }>(doc, {
      getRoot: (d) => d.getMap('root'),
    });
